...
app.registerExtension({
    name: "ComfyUI.Artha.Nodes.Artha.Flow.Pause",
	
	setup() {
        // Listen for reset messages from Python
        api.addEventListener("artha_flow_pause_button_reset", (event) => {
            const data = event.detail; 
            // Find all Project Pause nodes and reset their buttons
            const nodes = app.graph.findNodesByType("Artha Flow Pause");
            nodes.forEach(node => {
                if (node.button) {
					if(data.paused) {
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
		...
		if (nodeData.name === "ArthaFlowPause") {
            ...
            nodeType.prototype.onNodeCreated = function () {
				...
				this.button.onclick = () => {
					...
					fetch('/artha/flow_toggle_pause_button', {
						method: 'POST',
						headers: {
